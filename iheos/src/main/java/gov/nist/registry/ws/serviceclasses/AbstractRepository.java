...
abstract public class AbstractRepository extends XdsService  implements ContentValidationService {
	...
	public OMElement ProvideAndRegisterDocumentSetRequest(OMElement sor) throws AxisFault {
		try {
			OMElement startup_error = beginTransaction(getPnRTransactionName(), sor, XdsService.repository_actor);
	...
	public OMElement RetrieveDocumentSetRequest(OMElement rdsr) throws AxisFault {
		try {
			...
			if (ns_uri == null || ! ns_uri.equals(MetadataSupport.xdsB.getNamespaceURI())) {
				OMElement res = this.start_up_error(rdsr, "AbstractRepository.java", XdsService.repository_actor, "Invalid namespace on RetrieveDocumentSetRequest (" + ns_uri + ")", true);
