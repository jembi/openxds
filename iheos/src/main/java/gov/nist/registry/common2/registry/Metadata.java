...
public class Metadata {
	...
	public String getSimpleAssocType(OMElement assoc) throws MetadataException {
		...
		if (type == null) throw new MetadataException("Invalid Association object: no associationType attribute: " + assoc.toString());
	...
	public ArrayList getReferencedObjectsThatMustHaveSamePatientId() throws MetadataValidationException, MetadataException {
		...
		for (Iterator it=associations.iterator(); it.hasNext(); ) {
			...
			if (assoc_type(association_type).equals("HasMember") && "Reference".equals(getSlotValue(association, "SubmissionSetStatus", 0))) {
				continue;
			}
	...
	public OMElement mkAssociation(String type, String sourceUuid, String targetUuid) {
		OMElement assoc = MetadataSupport.om_factory.createOMElement("Association",MetadataSupport.ebRIMns2);
	...
	IdIndex id_index(Message log_message) throws MetadataException {
		if ( idIndex == null) {
			idIndex = new IdIndex();
