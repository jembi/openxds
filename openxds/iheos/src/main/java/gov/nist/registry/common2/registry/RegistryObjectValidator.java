...
public class RegistryObjectValidator extends StoredQuery {
	...
	public RegistryObjectValidator(ErrorLogger response, Message log_message) {
		super(response, log_message);
	}

	public ArrayList validateExists(ArrayList uuids)  throws XdsException, LoggerException {
		init();
	...
	public ArrayList<String> validateNotExists(ArrayList<String> ids)  throws XdsException, LoggerException {
			...
			if (dup_uids.contains(doc_uid)) {
				...
				if (sub_hash != null && reg_hash != null && !sub_hash.equals(reg_hash))
					response.add_error(MetadataSupport.XDSNonIdenticalHash,
							...
							"RegistryObjectValidation.java", log_message);
	...
	public ArrayList validateDocuments(ArrayList uuids)  throws  XdsException,LoggerException {
		init();
	...
	public ArrayList<String> validateAreFolders(ArrayList<String> ids) throws  XdsException,LoggerException {
		init();
	...
	public ArrayList validateApproved(ArrayList uuids)  throws  XdsException,LoggerException {
		init();
		a("SELECT * FROM ExtrinsicObject eo"); n();
		a("WHERE"); n();
	...
	public ArrayList validateSamePatientId(ArrayList uuids, String patient_id)  throws  XdsException,LoggerException {
		if (uuids.size() == 0)
			return null;
		init();
	...
	public ArrayList getXFRMandAPNDDocuments(ArrayList uuids) throws  XdsException, LoggerException {
		if (uuids.size() == 0)
			return new ArrayList();
		init();
		a("SELECT eo.id FROM ExtrinsicObject eo, Association a"); n();
		a("WHERE"); n();
		a("  a.associationType in ('XFRM', 'APND') AND"); n();
